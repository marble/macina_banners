	...
	function main($content, $conf) {
		...
		switch ((string)$conf['CMD']) {
			case 'singleView':
				...
				break;
			default:
				if (strstr($this->cObj->currentRecord, 'tt_content') && $this->cObj->data['list_type'] == 'macina_banners_pi1') {
			...
			*/
			foreach ($allowedPlacements AS $key => $placement) {
				if (t3lib_div::inList("top,bottom,right,left", $placement)) {
					...
					$catRow = $GLOBALS['TYPO3_DB']->exec_SELECTgetSingleRow('description','tx_macinabanners_categories', $catWhere);
					$allowedPlacements[$key] = $catRow['description'];
				}
				if ($placementClause != '')
		...
		for ($i=0; $i < $qt; $i++) {
			...
			switch($row['bannertype']) {
				...
				case 1:
					...
					#t3lib_utility_Debug::debug(array($clickTAG, $linkArray[1]));
					break;
